...
export default function App(){
 const [state,setState]=useState<AppState>("HOME"); const [files,setFiles]=useState<File[]>([]); const [language,setLanguage]=useState<OCRLanguage>("en"); const [result,setResult]=useState<AnalyzeResponse|null>(null);
 ...
 {state==="UPLOAD"&&<motion.div key="upload" className="flex-1" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0}}><Upload onFilesSelected={(f,l)=>{setFiles(f);setLanguage(l);nav("PROCESSING")}}/></motion.div>}
 {state==="PROCESSING"&&<motion.div key="processing" className="flex-1" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0}}><Processing files={files} language={language} onComplete={(r)=>{setResult(r);nav("RESULTS")}}/></motion.div>}
